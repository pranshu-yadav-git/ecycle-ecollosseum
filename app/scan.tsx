...
If the image doesn't show an electronic device, still return valid JSON but set deviceName to "Unknown Device", estimatedYear to 2015, and recycleValueINR to 500.`,
  ...
  const handlePickupConfirm = useCallback(
    (answers: ConditionAnswer[], address: string) => {
      if (!result || !scannedImage) return;
      const newPickup: SavedPickup = {
      ...
      showToast(`✅ Pickup confirmed for your ${result.brand} ${result.deviceName}! Agent arrives within 24 hours.`);
      resetScan();
    },
    [result, scannedImage]
  );
  ...
  const cancelPickup = useCallback((id: string) => {
    ...
    showToast('Pickup has been cancelled.');
  }, []);
          ...
          {pickups.length > 0 && (
            <View style={styles.pickupsSection}>
              <View style={styles.pickupsHeader}>
                <ThemedText style={styles.pickupsTitle}>My Pickups</ThemedText>
                <View style={styles.pickupCountBadge}>
                  <ThemedText style={styles.pickupCountText}>
                    {pickups.filter((p) => p.status === 'Confirmed').length} active
                  </ThemedText>
                </View>
              </View>
              {pickups.map((pickup) => (
                <PickupCard
                  key={pickup.id}
                  pickup={pickup}
                  onCancel={() => cancelPickup(pickup.id)}
                />
              ))}
            </View>
          )}
